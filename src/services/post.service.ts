...
export const getPostsService = async (req: Request, res: Response, _next: NextFunction) => {
  ...
  return response<any>({ data:responseObject, success: true, error: false, message: 'Successful Found posts', status: 200 });
};
...
export const createPostService = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    return response<any>({ data:createdPost, success: true, error: false, message: 'Successfully added new post', status: 201 });
  } catch (error) {
    console.log('error');
    return next(error);
  }
